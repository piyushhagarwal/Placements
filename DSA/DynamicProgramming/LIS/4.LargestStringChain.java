...
class Solution2 {
    ...
    public int solve(int currIndex, int prevIndex, String[] words, int[][] dp) {
        if (currIndex >= words.length) {
            return 0;
        }
        ...
        int include = Integer.MIN_VALUE;
        if (prevIndex == -1 || compare(words[currIndex], words[prevIndex])) {
            include = 1 + solve(currIndex + 1, currIndex, words, dp);
        }

        int exclude = solve(currIndex + 1, prevIndex, words, dp);
    ...
    public int longestStrChain(String[] words) {
        ...
        int currIndex = 0;
        int prevIndex = -1;
        int dp[][] = new int[words.length][words.length + 1]; // Here we are using nums.length + 1 because prevIndex can
        ...
        return solve(currIndex, prevIndex, words, dp);
    }
}